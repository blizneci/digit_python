...
9. Python.
   ...
   9.11 Словари/
    ...
    Задача 7
      ...
      разделенных пробелами (2, 4, 6, 8 или больше слов). Сформируй словарь таким
    ...
    Задача 10
      ...
      выведи исходный словарь. Для форматирования словаря при печати используй json.dumps.
      Sample Input:
   ...
   9.13 Области видимости переменных/
    ...
    Интерпретатор будет обращаться к объекту print(), который сначала найдет в локальной области и не
   ...
   9.18 Пакеты, модули/
    Задача 1
      ...
      7. from pprint import pprint as mypprint
        ...
        и обращаться к функции как pprint.pprint(var_name)
   ...
   9.21 Удобные няшки стандартной библиотеки/
    При работе с модулем csv нужно указывать delimiter=';', если планируется работать в Exel
    Гугл-таблицы нормально воспринимают в качестве delimiter запятую, которая используется по-умолчанию.
    Задача 1
...
10. Бэкэнд технологии.
   10.1 Веб-сервер и сервер приложений/
    Задача 1
      У нас есть сайт, серверная логика которого написана на Python.
      Какой блок серверных технологий выполняет этот Python код?
      1. Application сервер (gunicorn, например)
      2. Специальный сервер PostgreSQL
      3. Bash
      4. Nginx
        Ответ:
        1. Application сервер.
   10.2 CDN/
    Задача 1
      Мы находимся в Европе. Сервер, обслуживающий сайт А, находится в Америке.
      Сервер, обслуживающий сайт В, находится в Европе.
      Какой сайт будет для нас открываться быстрее при прочих равных?
      1. А
      2. В
        Ответ:
        2. В
    Задача 2
      Что можно кешировать в CDN?
      1. Python код
      2. Базы данных
      3. CSS файлы
      4. JS файлы
      ...
      7. HTML файлы
        Ответ:
        В CDN можно кешировать статичный файлы.
        CSS файлы, JS файлы, Видео, Картинки, HTML код
        Ответ на задачу
   10.3 Базы данных/
   10.4 Какую базу данных выбрать, SQL/
   10.5 Планировщики и очереди заданий/
    Задача 1
      Какая команда откроет настройки cron'а?
        Ответ:
        crontab -e
    Задача 2
      Напиши строку для файла конфигурации Crontab, которая будет запускать
      команду /usr/bin/myprogram каждое 10е число месяца ровно в 7 часов утра.
      Не используй символы табуляции и не разделяй символы более ем одним пробелом.
        Ответ:
        m h dom mon dow command
        0 7 10 * * /usr/bin/myprogram
    Задача 3
      Напиши команду, которая в постоянном режиме будет отслеживать последние
      строка файла server.log, находящегося в твоей домашней директрии, и
      выводить в консоль новые строки, добавленные в конец файла. Ты находишься
      в произвольной директории в Linux. Используй универсальный способ указать
      адрес домашней директрии текущего Linux пользователя.
        Ответ:
